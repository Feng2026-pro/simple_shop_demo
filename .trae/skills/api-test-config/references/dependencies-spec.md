...
与 api-test-runner 的 conftest 读取逻辑一致。用于配置前置依赖接口（如登录），执行 CSV 用例前会先调用这些接口并将 `extract` 产出注入到需要该依赖的用例请求中。
...
| body | 否 | 请求体对象。值可为环境变量占位，如 `${LOGIN_USER}`、`${LOGIN_PASS}`，运行时替换 |
...
- 请求体中的占位如 `LOGIN_USER`、`LOGIN_PASS` 等需在 `config/.env` 或环境中配置，不要在 yaml 中写明文密码。
- 新依赖使用的环境变量应在 `config/.env.example` 中列出示例；`config/.env` 为本地覆盖，可不提交。
- 建议默认在 `config/.env(.example)` 预置以下 DB 变量模板，避免后续启用 `db_assert` 时缺项：
  - `DB_ASSERT_ENABLED`（`false` 时跳过所有 DB 断言）
  - `DB_HOST`、`DB_PORT`、`DB_NAME`、`DB_USER`、`DB_PASS`
  - `DB_CHARSET`（默认 `utf8mb4`）
- 建议将 DB 变量集中维护在 `config/.env(.example)`；不要写进 `dependencies.yaml`。
...
## 示例 5：.env.example（含 DB 断言变量）

```dotenv
# API
BASE_URL=http://localhost:11011

# 前置依赖示例
LOGIN_USER=admin
LOGIN_PASS=your_password_here

# DB assert（建议默认预置）
DB_ASSERT_ENABLED=true
DB_HOST=127.0.0.1
DB_PORT=3306
